...
这个仓库已经不是单纯 demo，但也还不是完全收敛的成熟产品，当前更接近：

- 功能面很广的安全审计平台雏形
- 后端主干已打通：CLI、FastAPI、SQLite、规则系统、索引、调用链、污点分析、LLM、前端
- 部分功能仍处于实验性或演示性状态，尤其是 `api/graph_router.py`、`api/variant_router.py`、`api/server.py` 中的 Mock / in-memory 路径
- 核心风险不在“没功能”，而在“功能增长快于结构收敛”

后续协作时，默认按“真实产品代码 + 存在技术债”的标准处理，不要把 README 中的所有能力描述直接等同于当前稳定能力。
...
- 代码库不是零测试状态，但测试并非全绿
- 最近一次静态检查时，`pytest -q` 结果为 `62 passed, 15 failed`
- 失败主要集中在：
  - `indexer/indexer.py` 的 chunk 相关内部状态耦合
  - tree-sitter 解析器接口与测试契约不一致
  - 部分命名和数据结构演进后，测试未同步更新

结论：修改核心索引、解析、tree-sitter、fallback 逻辑前，必须先看相关测试是否已经失效，而不是假设代码本身一定错或测试本身一定错。
...
- **`indexer/`** - 代码解析（Python AST、Tree-sitter、部分正则回退）+ 向量存储 + 嵌入缓存
...
以下文件体量大、职责重，改动前必须完整阅读相关上下文：

- `api/main.py` - 主应用装配、状态管理、HTTP 路由、WebSocket、扫描编排
- `analyzer/engine.py` - 核心扫描与 LLM 分析主链路
- `indexer/indexer.py` - 文件遍历、解析、分块、缓存、索引流程
- `agent/unified_agent.py` - Agent 编排与工具调用
- `frontend/src/stores/auditStore.js` - 前端全局会话/聊天/WebSocket 状态

经验规则：

- 这些大文件里的“局部修复”很容易影响多条路径
- 修改函数签名、状态字段、返回结构时，必须反查 API、前端 store、测试三侧
- 如果能拆分文件，就不要继续往大文件里堆逻辑
...
- `storage/` 下的 SQLite 路径是当前真实持久化主干
- 但 `graph_router` / `variant_router` 仍有 in-memory 实现，不能误判为完整持久化能力
- `agent_router` 某些变体分析能力仍会通过 `api/server.py` 间接走到 Mock 分支

如果需求涉及“历史可追溯、结果可复现、重启不丢失”，优先检查是不是误用了实验性路由或 Mock 能力。
...
开始任何中大型改动前，优先检查以下高概率问题：

1. 是否碰到了实验性路由
   - `api/graph_router.py`
   - `api/variant_router.py`
   - `api/server.py`
2. 是否依赖 tree-sitter，但环境里并没有完整安装对应语言包
3. 是否修改了 `CodeUnit` / `Finding` / API schema / WebSocket payload，却没同步前端和测试
4. 是否在 `indexer/indexer.py` 中引入了新的共享状态，但没有处理并发锁和缓存失效
5. 是否把向量检索误当成主召回，而不是上下文补充
6. 是否在生产路径里残留 `print` / `console.log` / DEBUG 诊断代码

### 已知技术债

- `api/main.py` 过大，路由和编排耦合过深
- `analyzer/engine.py` 过大，承担过多策略分支
- `frontend/src/stores/auditStore.js` 状态过于集中，前端维护成本高
- 测试与实现存在契约漂移，尤其是 tree-sitter / fallback / chunk 逻辑
- README 展示的是目标能力，不能默认视为全部已经稳定实现
- `requirements.txt` 中 tree-sitter、pytest、ruff、black 仍是注释状态，环境复现性不足

### 推荐协作策略

- 先判断当前需求属于“稳定主干”还是“实验路径”
- 先修正确性，再修性能，再补交互
- 凡是涉及扫描结果可信度的改动，优先看：
  - `rules/`
  - `indexer/`
...
- 凡是涉及 UI 展示异常，优先同时看：
  ...
  - `frontend/src/stores/auditStore.js`

### 产品化方向

如果目标是把它从“功能很多的雏形”做成“广覆盖且可信的安全审计平台”，优先级应当是：
...
2. 提升可信度
   ...
   - 强化扫描结果的证据链和可解释性
3. 降低耦合
   - 拆分 `api/main.py`、`analyzer/engine.py`、`auditStore.js`
   - 将编排、领域模型、传输层解耦
4. 提升可运营性
   - 任务队列
   - 权限与多租户
...
这个仓库值得继续投入，但默认应按下面的心智模型协作：

- 它有真实的架构基础，不是玩具
- 它也有明显技术债，不要把任何单个实现视为“已经完全正确”
- 最重要的不是继续堆功能，而是把“扫描主链路、结果可信度、模块边界、测试契约”收敛起来

凡是和“漏洞是否真的存在”有关的逻辑，优先追求：
